...
public class JFinalWebInitializer implements IGroupInitializer {
    @Override
    public void init(Project project, Group groups) {
        File baseDir = new File(Config.targetPath() + FS + project.get("name"));
        File view = new File(Config.templatePath() + FS + project.getGroup().get("name") + FS + "common" + FS + "view" + FS + project.get("viewFramework") + FS
