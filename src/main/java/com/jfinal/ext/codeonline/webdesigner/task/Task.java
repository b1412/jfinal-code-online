package com.jfinal.ext.codeonline.webdesigner.task;

import com.jfinal.ext.codeonline.webdesigner.project.Project;
import com.jfinal.ext.codeonline.webdesigner.task.processor.ITaskProcessor;
...
public class Task extends ModelExt<Task> {
    ...
    public List<TaskParam> getTaskParams() {
        return TaskParam.DAO.find("select * from task_param where task_id = ?", getInt("id"));
    }

    public void deleteTaskParams() {
        Db.update("delete from task_param where task_id = ?", getInt("id"));
    }
    ...
    public boolean deleteById(Object id) {
        Db.update("delete from task_param where task_id =?",id);
